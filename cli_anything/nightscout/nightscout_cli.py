...
def _format_entry_row(e: dict[str, Any]) -> str:
    ...
    return f"  {ds:<28} {sgv!s:>5}  {direction}"
...
def entries_delete_by_type(
    ...
    if not apply_:
        _emit(
            ...
            human=f"[preview] {len(ids)} {type_filter} entries match; pass --apply to delete",
        )
        return
    if not _confirm(ctx, f"Delete {len(ids)} entries of type {type_filter!r}?", yes=yes):
        raise click.ClickException("aborted")
    if _dry_run_block(
        ctx,
...
@click.pass_context
def entries_slice(
    ctx: click.Context, storage: str, field: str, type_: str, prefix: str, regex: str
) -> None:
...
def food_list(ctx: click.Context, limit: int) -> None:
    ...
    if _is_json(ctx):
        ...
        for f in res:
            click.echo(
                f"  {f.get('food', '?'):<30} {f.get('carbs', '?')}g carbs / {f.get('portion', '?')} {f.get('unit', '')}"
            )
...
@click.option("--event-type", default=None, help="Filter by eventType (typically 'Exercise')")
...
@click.option("--event-type", default="Exercise", help="Activity event type (default 'Exercise')")
...
def activity_add(
    ...
    if _dry_run_block(
        ctx,
        "POST /v3/activity",
        payload={"eventType": event_type, "duration": duration, "notes": notes},
    ):
        return
...
@click.option(
    "--count", default=288, type=int, help="Entries to consider (288 ≈ 24h at 5min cadence)"
)
@click.option(
...
def report_tir(
    ...
    if date_gte or date_lte:
        data = entries_mod.list_entries(
            conn=conn, count=10000, type_="sgv", date_gte=date_gte, date_lte=date_lte
        )
    ...
    if _is_json(ctx):
        ...
        click.echo(
            f"  TIR ({lo}–{hi} {u}): {res['tir_pct']}%   ({res.get('in_range_count', '?')} readings)"
        )
        click.echo(
            f"  TBR (<{lo} {u}):           {res['tbr_pct']}%   ({res.get('below_count', '?')} readings)"
        )
        click.echo(
            f"  TAR (>{hi} {u}):           {res['tar_pct']}%   ({res.get('above_count', '?')} readings)"
        )
...
def report_daily(
    ...
    if _is_json(ctx):
        ...
        for r in res:
            ...
            click.echo(
                f"  {r['date']}   {r['count']:>5}   {mean!s:>5}     {mn!s:>5}  {mx!s:>5}   {r['tir_pct']:>5}"
            )
...
def report_agp(
    ...
    if _is_json(ctx):
        ...
        click.echo("  hour  count   p10    p25   p50   p75   p90    mean   TIR%")
        for r in rows:
            if r["count"] == 0:
...
@click.option("--days", default=14, type=int, help="Window size in days (default 14)")
@click.option(
    "--threshold", default=None, type=float, help="Hypo threshold (default: 70 mg/dL or 3.9 mmol/L)"
)
@click.option(
    "--min-duration", default=15, type=int, help="Minimum event duration in minutes (default 15)"
)
@click.option("--units", "units_flag", default=None, type=click.Choice(["mg/dl", "mmol", "mmol/l"]))
@click.option("--from", "date_gte", default=None, help="ISO date lower bound (overrides --days)")
@click.option("--to", "date_lte", default=None)
...
def report_hypos(
    ...
    if _is_json(ctx):
        ...
        click.echo(
            f"  Longest event: {longest['duration_min']:.0f}min "
        ...
        for e in evts:
            ...
            click.echo(
                f"  {e['start'][:19]:<22s} {e['duration_min']:>4.0f}m  {mn:>5}  {e['level']}"
            )
...
@click.option("--units", "units_flag", default=None, type=click.Choice(["mg/dl", "mmol", "mmol/l"]))
@click.option(
    "--tz", "tz_name", default=None, help="Weekday-boundary timezone (default: local system tz)."
)
@click.pass_context
def report_by_weekday(
    ctx: click.Context, days: int, units_flag: str | None, tz_name: str | None
) -> None:
    ...
    if _is_json(ctx):
        ...
        click.echo("  day   #     mean   TIR%   TBR%   TAR%")
        for r in rows:
            mean = r.get("mean_mmol") if mmol else r.get("mean_mgdl")
            click.echo(
                f"  {r['weekday']:<5s} {r['count']:>4d}  {mean!s:>5}  {r['tir_pct']:>5.1f}  {r['tbr_pct']:>5.2f}  {r['tar_pct']:>5.1f}"
            )
...
def report_excursions(
    ctx: click.Context, days: int, window_min: int, units_flag: str | None
) -> None:
    ...
    if _is_json(ctx):
        ...
        click.echo(
            f"  {'when':<19s} {'carbs':>5s} {'ins':>5s} {'base':>5s} {'peak':>5s} {'Δ':>5s} {'ttp':>5s} {'ICR':>5s}"
        )
        for r in rows[:50]:
            ...
            click.echo(
                f"  {r['created_at'][:19]:<19s} {r.get('carbs', '?')!s:>5} {r.get('insulin', '?')!s:>5} {base!s:>5} {peak!s:>5} {delta!s:>5} {r.get('time_to_peak_min', '?')!s:>5} {str(icr) if icr is not None else '—':>5}"
            )
...
@click.option("--units", "units_flag", default=None, type=click.Choice(["mg/dl", "mmol", "mmol/l"]))
@click.option(
    "--tz", "tz_name", default=None, help="Hour-of-day timezone (default: local system tz)."
)
@click.pass_context
def report_excursions_by_hour(
    ctx: click.Context, days: int, units_flag: str | None, tz_name: str | None
) -> None:
    ...
    txs = treatments_mod.list_treatments(conn=conn, count=10000, date_gte=iso_s, date_lte=iso_e)
    rows = excursions_mod.postprandial_responses(sgvs, txs, units=units, input_units="mg/dl")
    summ = excursions_mod.excursion_summary(rows, bucket="hour", tz=tz)
    if _is_json(ctx):
        _emit(ctx, summ)
    else:
        mmol_mode = _is_mmol_units(units)
        click.echo("  hour  meals  mean_baseline  mean_peak  mean_Δ  mean_ICR(g/U)")
        for r in summ:
            if mmol_mode:
            ...
            click.echo(
                f"  {r.get('hour', '?'):>3}h  {r.get('count', '?'):>5} {str(base) if base is not None else '—':>13} {str(peak) if peak is not None else '—':>10} {str(delta) if delta is not None else '—':>7} {str(icr) if icr is not None else '—':>13}"
            )
...
@click.option(
    "--at", "at_time", default=None, help="HH:MM time of day (default: now in local time)"
)
@click.pass_context
...
@click.option("--with-stats", is_flag=True, default=False, help="Include entry counts per session")
@click.pass_context
...
@click.pass_context
def v3_list_cmd(
    ctx: click.Context, collection: str, limit: int, sort: str | None, filters: tuple[str, ...]
) -> None:
    ...
    for raw in filters:
        if "=" not in raw:
            raise click.ClickException(f"--filter expects field$op=value, got {raw!r}")
        k, v = raw.split("=", 1)
        filter_dict[k.strip()] = v.strip()
    res = v3_mod.v3_list(collection, conn=conn, limit=limit, sort=sort, filter=filter_dict or None)
...
@click.option("--glucose-type", default="Finger", type=click.Choice(["Finger", "Sensor", "Manual"]))
@click.option("--notes", default=None)
@click.pass_context
def treatments_bg_check(
    ctx: click.Context, glucose: float, glucose_type: str, notes: str | None
) -> None:
    ...
    if _dry_run_block(
        ctx,
        "POST /treatments.json (BG Check)",
        payload={"glucose": glucose, "glucoseType": glucose_type, "notes": notes},
    ):
        return
    res = treatments_mod.add_bg_check(
        glucose=glucose, glucose_type=glucose_type, notes=notes, conn=conn
    )
    _maybe_save_session(ctx, action="treatments.bg_check", detail=f"{glucose} ({glucose_type})")
...
@click.option("--timeout", default=None, type=float, help="Stop after this many seconds")
@click.pass_context
def watch_entries_cmd(ctx: click.Context, timeout: float | None) -> None:
    ...
    def _cb(entry: dict) -> None:
        click.echo(
            f"[sgv] {entry.get('dateString', '?')[:19]}  sgv={entry.get('sgv', '?')}  dir={entry.get('direction', '?')}"
        )
...
def watch_treatments_cmd(ctx: click.Context, timeout: float | None) -> None:
    ...
    def _cb(t: dict) -> None:
        click.echo(
            f"[tx] {t.get('created_at', '?')[:19]}  {t.get('eventType', '?')}  carbs={t.get('carbs')}  ins={t.get('insulin')}"
        )
...
@click.pass_context
def v3_create_cmd(
    ctx: click.Context, collection: str, body_json: str | None, body_file: str | None
) -> None:
...
@click.option(
    "--field", "-f", multiple=True, help="Field(s) to apply --query to (default: notes,eventType)"
)
@click.option(
    "--filter",
    "raw_filter",
...
@click.argument("collection")
@click.option(
    "--since-ms", default=None, type=int, help="Epoch ms — return changes since this time"
)
@click.option("--limit", default=100, type=int)
@click.pass_context
def v3_history_cmd(ctx: click.Context, collection: str, since_ms: int | None, limit: int) -> None:
    """Fetch v3 collection mutation history (for incremental sync)."""
    conn = _conn(ctx)
    _require_url(conn)
    res = v3_mod.v3_history(collection, conn=conn, last_modified_ms=since_ms, limit=limit)
...
def treatments_update(
    ...
    if not fields:
        raise click.ClickException(
            "no fields to update — pass at least one of --carbs/--insulin/--glucose/--notes/--event-type/--created-at"
        )
...
def food_quickpicks(ctx: click.Context) -> None:
    ...
    if _is_json(ctx):
        ...
        for f in res:
            click.echo(
                f"  {f.get('food', '?'):<30} {f.get('carbs', '?')}g / {f.get('portion', '?')} {f.get('unit', '')}"
            )
...
def food_add(
    ...
    if _dry_run_block(
        ctx,
        "POST /v3/food",
        payload={"food": food, "carbs": carbs, "portion": portion, "unit": unit},
    ):
        return
    res = food_mod.add_food(
        food=food,
        carbs=carbs,
        portion=portion,
        unit=unit,
...
@click.option(
    ...
    "field",
    required=True,
    type=click.Choice(["basal", "carbratio", "sens", "target_low", "target_high"]),
)
@click.option("--at", "at_time", default=None, help="HH:MM (default: now)")
@click.option("--name", default=None, help="Named profile (default: active/defaultProfile)")
@click.pass_context
def profile_setting_at(
    ctx: click.Context, field: str, at_time: str | None, name: str | None
) -> None:
    ...
    if at_time is None:
        at_time = datetime.now().strftime("%H:%M")
    store = (
        profile_mod.current_named(name, conn=conn) if name else profile_mod.current_store(conn=conn)
    )
    if not store:
        _emit(ctx, {}, human="no active profile found")
        return
    ...
    _emit(
        ctx,
        {"field": field, "at": at_time, "value": value, "name": name or "(default)"},
...
@click.option("--body-file", default=None, type=click.Path(exists=True, dir_okay=False))
@click.pass_context
def profile_create(ctx: click.Context, body_json: str | None, body_file: str | None) -> None:
...
@click.option(
    "--body-json", default=None, help="JSON dict of field changes to merge onto the record"
)
@click.option("--body-file", default=None, type=click.Path(exists=True, dir_okay=False))
@click.pass_context
def profile_update(
    ctx: click.Context, profile_id: str, body_json: str | None, body_file: str | None
) -> None:
...
@click.option("--storage", default="entries")
@click.option("--field", default=None, help="Field name to filter on (e.g. 'type')")
@click.option("--op", default=None, help="Operator without leading $ (e.g. 'eq', 'gte')")
@click.option("--value", default=None, help="Value for the filter expression")
@click.pass_context
def entries_count(
    ctx: click.Context, storage: str, field: str | None, op: str | None, value: str | None
) -> None:
    """Server-side count over a storage collection."""
    conn = _conn(ctx)
    _require_url(conn)
    res = entries_mod.count_records(storage=storage, field=field, op=op, value=value, conn=conn)
...
@click.option("--from-units", default="mg/dl", type=click.Choice(["mg/dl", "mmol", "mmol/l"]))
@click.option("--count", default=50, type=int, help="How many recent entries to normalize")
@click.pass_context
def entries_normalize(ctx: click.Context, to_units: str, from_units: str, count: int) -> None:
...
@click.option("--created-at", default=None, help="Shortcut: ISO timestamp (default: now)")
@click.pass_context
def devicestatus_add(
    ctx: click.Context,
    body_json: str | None,
    body_file: str | None,
    device: str | None,
...
@click.option("--level", required=True, type=click.IntRange(0, 2), help="0=info, 1=warn, 2=urgent")
@click.option(
    "--time-minutes",
    type=int,
    default=None,
...
@click.pass_context
def notifications_ack(ctx: click.Context, level: int, time_minutes: int | None, group: str) -> None:
    """Acknowledge an outstanding alarm at the given urgency level."""
    conn = _conn(ctx)
    _require_url(conn)
    if _dry_run_block(
        ctx,
        f"POST /notifications/ack (level={level} group={group})",
        payload={"level": level, "time_minutes": time_minutes, "group": group},
    ):
        return
    res = notifications_mod.ack(level=level, time_minutes=time_minutes, group=group, conn=conn)
    _maybe_save_session(ctx, action="notifications.ack", detail=f"level={level} group={group}")
...
def notifications_admin(ctx: click.Context) -> None:
    ...
    if _is_json(ctx):
        ...
        click.echo(
            f"  {res.get('notifyCount', 0)} admin notice(s) "
...
@click.option("--days", default=30, type=int, help="History window for sensor-session detection")
@click.option(
    "--threshold-hours",
    default=168.0,
    type=float,
...
@click.pass_context
def report_sensor_life(ctx: click.Context, days: int, threshold_hours: float) -> None:
    ...
    if _is_json(ctx):
        ...
        if not cs:
            click.echo("  no sensor sessions detected in window")
            return
