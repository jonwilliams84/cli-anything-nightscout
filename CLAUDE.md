...
- Data trust (v2.5.0+): `report data-quality` scores CGM capture over the window you *asked for* (not the window the data happens to span) — run it before quoting any TIR/GMI/AGP, which all weight the readings they get equally and never notice a hole. `entries gaps` = just the dropouts; `--exclude-warmup` on data-quality discounts post-sensor-change silence from the denominator but still LISTS the gap as `explained: true`. `report accuracy` = meter-vs-sensor MARD/bias/%15-15/Clarke zones, pairing `mbg` entries + `BG Check` treatments against the nearest sgv; `glucoseType: Sensor` rows are excluded by default (grading the CGM with the CGM), and under `--min-pairs` (5) it returns `found: false` rather than a verdict. Clarke zone D = a real hypo the sensor called fine — it forces `urgent` regardless of MARD. `entries calibrations`/`entries raw` parse `cal` records and Nightscout's `rawbg`; no `cal` records is `found: false`, **not** a clean bill of health (Libre/Loop rigs never emit them), and an entry with no `unfiltered` gets `raw_mgdl: null` where Nightscout's own plugin would return 0.
- Loop history (v2.6.0+): `report loop` aggregates every loop/openaps devicestatus cycle in a window — cadence, enacted %, failure histogram, IOB/COB at decision time and the basal the loop commanded. `devicestatus loop` stays the latest-cycle snapshot. No loop documents in the window is `found: false`/`unknown`, never a healthy loop; missing IOB/COB fields are `present: 0`, not 0; staleness (default warn 30 / urgent 60 min, `--stale-minutes`) and < 50% enactment over ≥ 4 cycles raise the `level`. `commanded_basal` is loop intent, not confirmed delivery — cross-check with `report basal`.
